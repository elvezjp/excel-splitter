...
バージョンを上げるときは、以下のファイルを整合するように更新します。

- `pyproject.toml`: `version` の値を更新
- `CHANGELOG.md` と `CHANGELOG_ja.md`: `[Unreleased]` の変更をリリースバージョンと日付へ移動
- `SECURITY.md` と `SECURITY_ja.md`: サポート対象バージョンの範囲が変わる場合は更新
